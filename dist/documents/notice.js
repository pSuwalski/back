...
function generateNoticePaper(id, union, company, owner, resolution, stampAuth, area, accountName, address, bankAccount, title, status) {
  ...
  <body>
    ...
    <div class="table-container">
      ...
      <tr>  
        ...
        ${generateRows(owner.fees.filter((f) => f.cause.indexOf(`uchawały ${resolution.number} na rok ${resolution.year}`) === -1), (owner.payments ? owner.payments : []).filter((p) => p.type === 'additionalCosts'))}   
      </table>
    </div>
...
exports.generateNoticePaper = generateNoticePaper;
function generateRows(fees, additionalCosts) {
  ...
  <td class="column table-left-quarter" > ${f.value + (f.type === 'fee' ? f.intrests : 0)} </td>
    ...
    const moreThanThreeYearsSum = allFees.filter((f) => 3 > new Date().getFullYear() - f.forYear).reduce((acc, v) => {
        if (v.type === 'fee') {
            acc.capital += v.value;
